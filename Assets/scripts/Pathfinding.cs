...
public class Pathfinding : MonoBehaviour
{
	...
	Vector3[] SimplifyPath(List<GridObject> path)
	{
		...
		waypoints.Add(path[0].grid.GetWorldPosition(path[0].getX(), path[0].getY()) + Vector3.up * 3);
		...
		for (int i = 1; i < path.Count; i++)
		{
			...
			{
				waypoints.Add(path[i - 1].grid.GetWorldPosition(path[i - 1].getX(), path[i - 1].getY()) + Vector3.up * 3);
